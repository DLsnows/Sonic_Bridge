...
const TOPICS: Record<string, Topic> = {
  ...
  project: {
    ...
    body: [
      ...
      "",
      "`use` accepts an exact customId, a full UUID, or a unique 8-character",
      "prefix (the same shape `ls` prints). Stores the selection in the config so",
      "subsequent commands can omit --project. Ambiguous prefix fails clearly.",
    ].join("\n"),
  },
