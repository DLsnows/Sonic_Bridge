...
const TOPICS: Record<string, Topic> = {
  ...
  files: {
    ...
    body: [
      ...
      "to stdout. `mv --to root` moves the file out of any folder. `rename` keeps",
      "the file's extension; trying to change it returns 422. `rm` prompts for",
      "the user's password and then issues a single-use challenge token.",
    ].join("\n"),
  },
  folders: {
    name: "folders",
