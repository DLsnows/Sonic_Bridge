...
const TOPICS: Record<string, Topic> = {
  login: {
    ...
    body: [
      "Usage:",
      "  sonicbridge login",
      "  sonicbridge login --base-url <url> --token <sb_...>",
      "",
  ...
  files: {
    name: "files",
    summary: "List, upload, download, move, and delete project files.",
    body: [
      "Usage:",
      ...
      "",
      "`upload` uses the two-step presigned URL flow. `download --out -` streams",
      "to stdout. `mv --to root` moves the file out of any folder. `rm` prompts",
      "for the user's password and then issues a single-use challenge token.",
    ].join("\n"),
  },
  folders: {
    name: "folders",
    summary: "Browse, create, and remove folders.",
    body: [
      "Usage:",
      ...
      "prints the files inside that folder. `rm` refuses non-empty folders with",
