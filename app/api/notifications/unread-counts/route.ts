...
export async function GET() {
  try {
    ...
    try {
      const views = await db.select().from(projectViews)
        .where(and(eq(projectViews.userId, userId), inArray(projectViews.projectId, projectIds)));
    ...
    for (const pid of projectIds) {
      ...
      // reply_to_user notifications (exclude new_post to avoid double-count with source threads)
      try {
      ...
      if (total > 0) {
        counts[pid] = { total, threads: threadCount, files: fileCount, events: eventCount };
      }
    }
