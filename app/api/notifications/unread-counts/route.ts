...
export async function GET() {
  try {
    ...
    for (const pid of projectIds) {
      ...
      try { const conds = [eq(discussionPosts.projectId, pid), isNull(discussionPosts.parentId), ne(discussionPosts.isAiGenerated, true), ne(discussionPosts.userId, userId)]; if (since) conds.push(gt(discussionPosts.createdAt, since)); const [r] = await db.select({ c: count() }).from(discussionPosts).where(and(...conds)); threadCount = r?.c ?? 0; } catch { /* nf */ }
      ...
      try { const conds = [eq(files.projectId, pid), ne(files.uploadedBy, userId)]; if (since) conds.push(gt(files.uploadedAt, since)); const [r] = await db.select({ c: count() }).from(files).where(and(...conds)); fileCount = r?.c ?? 0; } catch { /* nf */ }
      try { const conds = [eq(scheduleEvents.projectId, pid), gte(scheduleEvents.startTime, new Date()), ne(scheduleEvents.createdBy, userId)]; if (since) conds.push(gt(scheduleEvents.createdAt, since)); const [r] = await db.select({ c: count() }).from(scheduleEvents).where(and(...conds)); eventCount = r?.c ?? 0; } catch { /* nf */ }
