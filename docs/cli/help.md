...
sonicbridge <command> [args] [flags]
  ...
  files ...          Manage project files (ls, upload, download, mv, rm)
  folders ...        Manage folders (ls, mkdir, rm)
...
- `files rm` — **destructive.** Prompts for your password, mints a short-lived single-use challenge, then deletes. The challenge is only consumed if the file exists, so typoing the id costs nothing. Rate-limited to 5 failed password attempts per 15 minutes.
